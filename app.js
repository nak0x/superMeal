...
async function loadFilters(){
    FILTERS.forEach(async filter => {
        ...
        if(!filterOptions || !filterOptions.meals){return}
...
async function loadAmazon(filter=null){
    ...
    if(filter === null){
        ...
        for(let i = 0; i < 12; i++){
            ...
            if(!meal || !meal.meals){return mealsContainer.innerHTML = "There is nothing here !";}
            meal = meal.meals[0]
    ...
    if(!meals || !meals.meals){return mealsContainer.innerHTML = "There is nothing here !";}
    meals = meals.meals;
    mealsContainer.innerHTML = ""
    meals.forEach(meal => {
        mealsContainer.innerHTML += getHtmlParsedMeal(meal);
